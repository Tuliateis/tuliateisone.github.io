...
class DayCommon1 {
    ...
    loadWardContent(room) {
        // 第15天特殊逻辑
        if (this.dayData.specialDay15 && this.dayData.specialDay15.enabled && room === '202') {
            this.handleDay15SpecialEvent(room);
            return;
        }
    ...
    handleDay15SpecialEvent(room) {
        console.log('开始处理第15天特殊事件');
        
        const specialConfig = this.dayData.specialDay15.room202Trigger;
        
        // 先显示正常病房内容
        this.loadNormalWardContent(room);
        // 重要：显示对话！
        setTimeout(() => {
            console.log('尝试显示202病房对话');
            this.showDialogue(room);
        }, 1000);
        // 4.987秒后开始特殊事件
        setTimeout(() => {
            this.startDay15Transformation(specialConfig);
        }, specialConfig.delay);
    }

    // 加载正常病房内容（临时）
    loadNormalWardContent(room) {
        const wardDetails = document.getElementById('ward-details');
        if (!wardDetails) return;
        ...
        // 设置图片
        const imageContainer = wardDetails.querySelector('.patient-image-container');
        if (imageContainer && this.dayData.patientImages && this.dayData.patientImages[room]) {
            imageContainer.style.backgroundImage = `url(${this.dayData.patientImages[room]})`;
            imageContainer.style.backgroundSize = 'cover';
            imageContainer.style.backgroundPosition = 'center';
        }
        // 绑定检查按钮事件
        const examineBtn = document.getElementById(`examine-btn-${room}`);
        if (examineBtn) {
            examineBtn.addEventListener('click', () => this.performExamination(room));
        }
    }

    // 开始第15天转换

    startDay15Transformation(config) {
        console.log('开始第15天转换流程');
        
        // 1. 逐渐变黑
        this.startBlackoutEffect()
            .then(() => {
                console.log('变黑完成，开始纯黑停留');
                // 2. 纯黑界面停留1.5秒
                return new Promise(resolve => setTimeout(resolve, config.blackoutDuration));
            })
            .then(() => {
                console.log('纯黑停留结束，开始变亮并重新布局');
                // 3. 逐渐变亮并重新布局
                return this.fadeInAndRedesign();
            })
            .then(() => {
                console.log('重新布局完成，显示检查选项');
                // 4. 显示检查选项
                this.showExaminationOption();
            })
            .catch(error => {
                console.error('第15天转换出错:', error);
            });
    }
    ...
    startBlackoutEffect() {
        return new Promise((resolve) => {
            ...
            overlay.id = 'day15-blackout-overlay';
    ...
    fadeInAndRedesign() {
        return new Promise((resolve) => {
            ...
            // 修改员工休息室为202病房
            this.renameStaffRoom();
            
            // 替换图片为202end.png
            this.replaceRoomImage();
            
            // 逐渐变亮
            const overlay = document.getElementById('day15-blackout-overlay');
            if (overlay) {
    ...
    removeTabs() {
        ...
        if (mainTabs) {
            // 只保留员工休息室标签，但改名为202病房
            const staffRoomTab = document.getElementById('tab-staff-room');
            if (staffRoomTab) {
                mainTabs.innerHTML = '';
                const newTab = staffRoomTab.cloneNode(true);
                newTab.textContent = '202病房';
                newTab.id = 'tab-202-room';
                newTab.setAttribute('data-page', 'staff-room');
                mainTabs.appendChild(newTab);
            }
        }
        ...
        const staffRoomPage = document.getElementById('page-staff-room');
        if (staffRoomPage) {
            staffRoomPage.classList.add('active');
        }
    }
    ...
    replaceRoomImage() {
        ...
        if (staffRoomWrapper) {
            ...
            newImage.id = 'staff-room-bg';
            newImage.className = 'staff-room-bg';
            newImage.src = 'images/202end.png';
            newImage.alt = '202病房';
            newImage.onerror = () => {
                console.error('202end.png加载失败，使用备用图片');
                newImage.src = 'images/room.jpg';
            };
            
            staffRoomWrapper.appendChild(newImage);
            console.log('图片替换为202end.png');
        }
    }
    // 重命名员工休息室
    renameStaffRoom() {
        const staffRoomTab = document.getElementById('tab-staff-room');
        if (staffRoomTab) {
            staffRoomTab.textContent = '202病房';
            staffRoomTab.id = 'tab-202-room';
        }
    }
    // 重新设计界面
    redesignInterface() {
        return new Promise((resolve) => {
            // 删除前台、病房页签
            const mainTabs = document.querySelector('.main-tabs');
            if (mainTabs) mainTabs.innerHTML = '';
            
            // 修改员工休息室为202病房
            const staffRoomTab = document.getElementById('tab-staff-room');
            if (staffRoomTab) {
                staffRoomTab.textContent = '202病房';
                staffRoomTab.setAttribute('data-page', 'ward-202');
            }
            
            // 切换到"202病房"页签
            setTimeout(() => {
                if (staffRoomTab) staffRoomTab.click();
                this.setup202RoomInterface();
                resolve();
            }, 500);
        });
    ...
    // 设置202病房特殊界面
// 设置202病房特殊界面
    setup202RoomInterface() {
        const staffRoomBg = document.getElementById('staff-room-bg');
        if (!staffRoomBg) {
            console.error('员工休息室图片未找到');
            return;
        }
        
        // 创建选项覆盖层
        const optionsOverlay = document.createElement('div');
        optionsOverlay.id = 'day15-options-overlay';
        ...
        question.style.cssText = `
            ...
            font-weight: bold;
        `;
        question.textContent = '是否接受例行检查';
        
        const buttons = document.createElement('div');
        buttons.id = 'day15-buttons';
        ...
        buttons.innerHTML = `
            <button class="day15-btn" style="padding: 1rem 2rem; font-size: 1.3rem; background: #3584e4; color: white; border: none; border-radius: 5px; cursor: pointer;">接受</button>
            <button class="day15-btn" style="padding: 1rem 2rem; font-size: 1.3rem; background: #3584e4; color: white; border: none; border-radius: 5px; cursor: pointer;">接受</button>
        `;
        
        optionsOverlay.appendChild(question);
        optionsOverlay.appendChild(buttons);
        
        // 添加到图片容器
        const staffRoomWrapper = document.querySelector('#page-staff-room .staff-room-wrapper');
        if (staffRoomWrapper) {
            staffRoomWrapper.style.position = 'relative';
            staffRoomWrapper.appendChild(optionsOverlay);
        }
        
        // 绑定按钮事件
        setTimeout(() => {
            document.querySelectorAll('.day15-btn').forEach(btn => {
                btn.addEventListener('click', () => {
                    this.acceptExamination();
                });
            });
        }, 100);
        
        console.log('202病房特殊界面设置完成');
    }
    // 设置202病房点击区域
    setup202RoomClickAreas() {
        const staffRoomBg = document.getElementById('staff-room-bg');
        ...
        staffRoomBg.addEventListener('click', (e) => {
            ...
            // 中左区域（左侧40%宽度，中间60%高度）
            if (x < width * 0.4 && y > height * 0.2 && y < height * 0.8) {
                this.handle202RoomInteraction();
            }
        });
    }

    // 显示检查选项
    showExaminationOption() {
        const question = document.getElementById('day15-question');
        const buttons = document.getElementById('day15-buttons');
        
        if (question && buttons) {
            question.innerHTML = '是否接受例行检查';
            buttons.innerHTML = `
                <button class="day15-btn" onclick="window.dayCommonInstance.acceptExamination()">接受</button>
                <button class="day15-btn" onclick="window.dayCommonInstance.acceptExamination()">接受</button>
            `;
        }
    }
    ...
    acceptExamination() {
        ...
        if (question) {
            question.textContent = '检查中';
        }
        if (buttons) {
            buttons.innerHTML = '';
        }
        
        const config = this.dayData.specialDay15.room202Trigger;
        
        setTimeout(() => {
            this.showExaminationResults();
        }, config.examinationDelay);
    }
    ...
    showExaminationResults() {
        ...
        if (question) {
            question.innerHTML = `
                <div style="text-align: left; line-height: 1.8;">
                    <div>体温: 36.8°C</div>
                    <div>血压: 110/70 mmHg</div>
                    <div>脉搏: 60 bpm</div>
                    <div>表现: 面部露出一种恐惧的神态，但肉质紧实鲜嫩</div>
                </div>
            `;
        }
        ...
        setTimeout(() => {
            this.showTreatmentOption();
        }, config.treatmentDelay);
    }
    ...
    showTreatmentOption() {
        ...
        if (question && buttons) {
            question.innerHTML = '是否接受治疗：注射■药，手术：摘除心脏';
    ...
    acceptTreatment() {
        ...
        // 闪白效果
        this.flashWhiteEffect()
            .then(() => {
                // 逐渐变黑
                return this.startBlackoutEffect();
            })
            .then(() => {
                // 显示结局
                this.showDay15Ending();
            });
    }
    ...
    showDay15Ending() {
        ...
        container.id = 'day15-ending-container';
        ...
        // 逐字打印结局
        this.typewriterEnding(container, settlementData);
    }


    // 逐字打印结局 - 修改版本

    typewriterEnding(container, settlementData) {
        // 清空容器
        container.innerHTML = '';
        ...
        // 逐字打印第一行
        this.typeText(line1, settlementData.firstLine, 100)
            .then(() => {
                // 逐字打印第二行
                return this.typeText(line2, settlementData.secondLine[0], 80);
            })
            .then(() => {
                // 第二行打印完成后停顿1秒
            ...
            .then(() => {
                ...
                // 显示圆圈
                return this.showDots(dotsContainer, settlementData.dots);
            })
            .then(() => {
                // 停留2秒
                return new Promise(resolve => setTimeout(resolve, settlementData.endingDelay));
            })
            .then(() => {
                // 清空所有内容，显示最终消息
                container.innerHTML = '';
                ...
                return this.typeText(finalMessage, settlementData.finalMessage, 80);
            })
            .then(() => {
                // 跳转到首页
                setTimeout(() => {
                    window.location.href = settlementData.redirectUrl;
                }, settlementData.redirectDelay);
            });
    }

    // 修改typeText方法，支持在现有内容后追加
    typeText(element, text, speed) {
        return new Promise((resolve) => {
            let index = 0;
            const originalText = element.innerHTML; // 保存原有内容
            
            function type() {
                if (index < text.length) {
                    element.innerHTML = originalText + text.substring(0, index + 1);
                    index++;
        ...
        return new Promise((resolve) => {
            ...
            function addDot() {
                if (index < dots.length) {
                    ...
                    dot.textContent = '●';
                    dot.style.color = dots[index] === 'red' ? '#ff0000' : '#ffffff';
                    dot.style.margin = '0 0.5rem';
                    dot.style.textShadow = dots[index] === 'red' ? '0 0 10px #ff0000' : 'none';
