...
class DayCommon2 {
    ...
    loadWardContent(room) {
        ...
        // ——【关键】切换病房时销毁旧的眼睛跟随实例
        if (this.eyeFollowInstance) {
            this.eyeFollowInstance.destroy();
            this.eyeFollowInstance = null;
        }
        ...
        const isExamined = this.gameState.examinedPatients?.[room] || false;
        const hasTreatment = this.gameState.patientTreatments?.[room] || false;

        const medicineCheck = this.checkAllMedicinesTaken(room);
        const canTreat = isExamined && medicineCheck.allTaken;

        // ——【图片路径处理开始】
        const imageConfig = this.dayData.patientImages?.[room];

        let imagePath = '';
        let enableEyeFollow202 = false;

        if (room === '202' && typeof imageConfig === 'string') {
            // 202：使用文件夹下的 center.png
            imagePath = `${imageConfig}/202nd.png`;
            enableEyeFollow202 = true;
        } else if (typeof imageConfig === 'string') {
            // 普通病人：直接图片路径
            imagePath = imageConfig;
        } else {
            // 兜底（你必须保证这个文件存在）
            ...
            <div class="patient-detail-container">
                <div class="patient-info-section">
                    ...
                    <div class="examine-section" id="examine-section-${room}">
                        ${!isExamined ? `<button class="examine-btn" id="examine-btn-${room}">例行检查</button>` : ''}
                    </div>
                    ...
                    <div class="vitals-container ${isExamined ? 'show' : ''}" id="vitals-${room}" style="margin-top:30px;">
                        ...
                        <div class="vital-row"><span class="vital-label">表现</span><span class="vital-value">${patient.vitals.symptoms}</span></div>
                    </div>
                </div>
                ...
                <div class="treatment-form-section">
                    ...
                    <form class="treatment-form" id="treatment-form-${room}">
                        ...
                        <button
                            type="button"
                            class="submit-treatment"
                            id="submit-treatment-${room}"
                            style="${hasTreatment ? 'background-color:#32cd32;' : ''}"
                        >
        ...
        if (imageContainer) {
            ...
            // ——【只对 202 启用眼睛跟随】
            if (enableEyeFollow202) {
                this.eyeFollowInstance = new PatientEyeFollow202(
                    imageContainer,
                    imageConfig // images/202nd
                );
            }
        }

        if (!isExamined) {
            const btn = document.getElementById(`examine-btn-${room}`);
            if (btn) btn.addEventListener('click', () => this.performExamination(room));
        }

        // ——始终绑定治疗表单点击逻辑
        this.bindTreatmentForm(room, hasTreatment);

        // ——如果不能治疗，仅在视觉上限制
        if (!canTreat) {
            const form = wardDetails.querySelector('.treatment-form');
            if (form) {
        ...
        setTimeout(() => this.showDialogue(room), 300);
    }
    ...
    goToNextDay(currentDay) {
        ...
        if (nextDay <= 19) {
            ...
            if (navigation.onGameComplete) {
                ...
                this.showGameModal('通关', '恭喜完成所有15天的工作！');
            }
        }
    }
